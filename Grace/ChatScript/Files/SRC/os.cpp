...
void C_Directories(char* x)
{
	...
	int bytes;
#ifdef WIN32
	bytes = GetModuleFileName(NULL, (LPWSTR)word, len);
#else
	char szTmp[32];
...
void WalkDirectory(char* directory,FILEWALK function, uint64 flags) 
{
	...
	{
		if (FindFileData.cFileName[0] != '.' && stricmp((char*)FindFileData.cFileName,(char*)"bugs.txt"))
		{
		...
		{
			if (FindFileData.cFileName[0] == '.' || !stricmp((char*)FindFileData.cFileName,(char*)"bugs.txt")) continue;
			sprintf(name,(char*)"%s/%s",directory,FindFileData.cFileName);
			(*function)(name,flags);
		}
