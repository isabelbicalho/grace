...
{ // Javascript permanent {call void name type type} eval ...code...
	...
	*name = 0;
	...
	if (!strcmp(word,"call")) // skip over the description for now
	{
		...
		code = definition;
		while (!strcmp(word,"string") || !strcmp(word,"int")  || !strcmp(word,"float")) 
		{
	...
	if (*code && !inited) // code was supplied, handle it if not yet executed
	{
		*defstart = '.';
		...
		if (!strcmp(file,"file")) // read files
		{
			while ((code = ReadCompiledWord(code,file)) != NULL && !strcmp(file,"file"))
			{
	...
	if (*name)
	{
		...
		{
			if (!strcmp(returnType,"string")) 	strcpy(buffer,duk_safe_to_string(ctx, -1)); // assumes there is a return string!
			else if (!strcmp(returnType,"int")) strcpy(buffer,duk_safe_to_string(ctx, -1)); // assumes there is a return string!
			else if (!strcmp(returnType,"float")) strcpy(buffer,duk_safe_to_string(ctx, -1)); // assumes there is a return string!
			duk_pop(ctx);
